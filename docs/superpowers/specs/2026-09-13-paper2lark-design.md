...
**M6 implementation amendment (2026-09-16):** Version 0.7.0 implements deterministic local-marketplace ZIP distribution, SHA-256 manifests, English host installation/upgrade documentation and a nine-combination core CI definition. The M0 standard-library zipapp amendment remains in force: no wheel/venv bootstrap or optional reader is bundled. The stronger 256 KiB per-plugin cap remains enforced. Windows Claude Code/Codex CLI archive installation and external-state-preserving replacement have direct evidence; archive workflows use synthetic providers. Fresh model-driven/live Lark end-to-end acceptance and unexecuted OS/host combinations remain explicit public-release gates, not inferred successes. See `docs/compatibility-m6.md` and `docs/release-checklist.md`.
