...
The SQLite schema remains v3. A valid 0.7.0 layout and unfinished run remain readable because this patch adds no database migration or run-version pin. Setup plans deliberately retain exact runtime matching: a 0.7.0 plan must be recovered with its original runtime, which should be retained during upgrade.
