# Install Paper2Lark for Claude Code
...
Download the Claude ZIP from the [release assets](https://github.com/William-f-12/Paper2Lark/releases), compare its SHA-256 with the release checksum file, and extract it into a dedicated, versioned directory. The extracted root must contain `.claude-plugin/marketplace.json` and `plugins/paper2lark/`. Use the complete root; a plugin subdirectory alone is not a marketplace. The source repository root is not a built marketplace.

In Claude Code started from that extracted root, run:
...
Select the intended installation scope. Start a new session, then invoke `/paper2lark:probe`. This local probe needs no Lark authorization. Other skills are `/paper2lark:doctor`, `/paper2lark:setup`, `/paper2lark:add`, `/paper2lark:library`, and `/paper2lark:read`.

For a temporary session without marketplace registration, from the extracted root:

```powershell
claude --plugin-dir ./plugins/paper2lark
```

This alternative lasts for that session; it does not establish persistent installation.
...
Command syntax was checked against Claude Code 2.1.270 and the [official installation documentation](https://code.claude.com/docs/en/discover-plugins) on 2026-09-16. Documentation verification is separate from release certification.
